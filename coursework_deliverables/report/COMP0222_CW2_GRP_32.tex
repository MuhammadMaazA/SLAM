...
and GTSAM factor-graph optimisation reduces trajectory closure error by
15--62\% on indoor sequences.
...
\midrule
KITTI 07       & 4.39\,m & 5.11\,m & \textcolor{red}{$+16\%$}\\
TUM long office & 0.032\,m & 0.038\,m & \textcolor{red}{$+19\%$}\\
\bottomrule
\end{tabular}
...
disabling loop closure increases ATE by 16\%.  TUM's office sequence
is a partial loop: removing closure correction causes a modest 19\% ATE
increase.  The effect is smaller than outlier rejection because TUM is
short enough that drift without closure is not severe.  Figure~\ref{fig:q1d}
shows how the uncorrected trajectory diverges from the corrected path.
...
We use an RPLidar A1 (360$^\circ$, 10\,Hz, 12\,m rated range) to record
...
edges carry their own ICP Hessian propagated through detection (correcting
the prior bug where all loop edges used a fixed sigma regardless of match
quality).  Pose-0 is anchored with a tight Gaussian prior ($\sigma =
...
\midrule
Floor7\_Hallway & 0.163\,m & 0.061\,m & $\mathbf{-62\%}$\\
Basement\_1     & 0.194\,m & 0.165\,m & $-15\%$\\
Outdoor\_1      & 0.941\,m & 0.886\,m & $-6\%$\\
\bottomrule
\end{tabular}
\caption{Q3d: Closure error before and after factor-graph optimisation.}
\label{tab:q3d}
\end{table}

Floor7\_Hallway benefits most (62\% reduction) because the large-area
corridor layout provides spatially separated loop revisits that strongly
constrain the drift direction.  Basement\_1 improves by 15\%; the small
reduction is expected since the loop geometry is less over-determined.
Outdoor\_1 sees 6\% improvement; sparse geometry makes ICP loop-closure
estimates noisier.
